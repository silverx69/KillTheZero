		...
		if (prop.isArray) {
			if (prop.arraySize > 0) {
				...
				for (int i = 0; i < prop.arraySize; i++)
					if (!SelectPrefabWarning(prop.GetArrayElementAtIndex(i).objectReferenceValue as GameObject))
						b = false;
				return b;
            }
			else return SelectPrefabWarning(null);
		}
		else return SelectPrefabWarning(serializedObject.FindProperty(propertyName).objectReferenceValue as GameObject);
	}

	private bool SelectPrefabWarning(GameObject go) {
		if (go != null) {
			if (!string.IsNullOrEmpty(go.scene.name))
				ShowPrefabSceneWarning();
		}
		else {
			ShowPrefabSelectWarning();
			return false;
		}
		return true;
	}

	private void ShowPrefabSceneWarning() {
		GUILayout.Space(preWarningSpace);
		EditorGUILayout.HelpBox(prefabNotSceneHint, MessageType.Warning);
	}

	private void ShowPrefabSelectWarning() {
		GUILayout.Space(preWarningSpace);
