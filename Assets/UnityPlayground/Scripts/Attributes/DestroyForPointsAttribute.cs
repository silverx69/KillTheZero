...
public class DestroyForPointsAttribute : MonoBehaviour
{
	...
	private void OnTriggerEnter2D(Collider2D collisionData)
	{
		...
		if(collisionData.gameObject.CompareTag("Bullet"))
		{
			...
			else Debug.Log("Use a BulletAttribute on one of the objects involved in the collision if you want one of the players to receive points for destroying the target.");
		}
	}
}
